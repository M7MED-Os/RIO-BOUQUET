...
export default function ImageGallery({ images, productName }: ImageGalleryProps) {
  ...
  if (validImages.length === 0) {
    return (
      <div className="flex aspect-square w-full items-center justify-center rounded-3xl bg-rose-50 border border-rose-100">
  ...
  return (
    <div className="space-y-3">
      {/* Main image */}
      <div className="relative aspect-square w-full overflow-hidden rounded-3xl bg-rose-50 shadow-xl border border-rose-100">
        <Image
        ...
        {/* Rating badge */}
        <div className="absolute top-4 right-4 flex items-center gap-1 rounded-full bg-white/90 px-3 py-1.5 text-xs font-bold text-amber-500 shadow-md backdrop-blur-sm">
          ⭐ تقييم ممتاز
        </div>
      </div>
          ...
          {validImages.map((url, index) => (
            <button
              ...
              className={`relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 transition-all ${
                index === activeIndex
                  ? 'border-rose-600 shadow-md shadow-rose-200 scale-105'
                  : 'border-zinc-200 hover:border-rose-300 opacity-70 hover:opacity-100'
              }`}
            >
